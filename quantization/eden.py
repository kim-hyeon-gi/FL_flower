...
from quantization.base import CompressionPipeline, Transform, flatten
from quantization.hadamard import RandomizedHadamard, padToPowerOf2
from quantization.quantization_constants import (
    QuantizationType,
    get_all_quantization_constants_tensors,
)
from quantization.random_p import RandomP


class EdenQuantization(Transform):
    def __init__(self, bits, q_type: QuantizationType = "max_lloyd", device="cuda"):

        self.centroids, self.boundaries = get_all_quantization_constants_tensors(
            q_type, device
        )
    ...
    def forward(self, x):
        ...
        if self.fractional_bits:
            ...
            unscaled_centers_vec_low = torch.take(
                self.centroids[self.bits_low], assignments_low
            )
            unscaled_centers_vec_high = torch.take(
                self.centroids[self.bits_high], assignments_high
            )

            unscaled_centers_vec = mask_combine(
                unscaled_centers_vec_low, unscaled_centers_vec_high, mask
            )
...
def bernoulli_mask(shape, device, p, prng):
    return torch.empty(shape, dtype=torch.bool, device=device).bernoulli_(
        p=p, generator=prng
    )
...
def eden_builder(bits, q_type: QuantizationType = "max_lloyd", device="cuda"):
    """
